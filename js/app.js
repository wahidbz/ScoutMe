...
const boot = async () => {
  // 1️⃣ تحميل اللغة المختارة مسبقًا أو الإنجليزية
  const lang = localStorage.getItem('scoutme_language') || 'en';
  await loadLanguage(lang);

  // 2️⃣ تهيئة الـ language switcher بدون انتظار profile
  const langSelect = document.getElementById('global-language');
  if (langSelect) {
    setupLanguageSwitcher(langSelect, async (newLang) => {
      await loadLanguage(newLang);
      applyTranslations(document);
      // إذا كان هناك صفحة محمية بها profile
      const page = document.body.dataset.page;
      if (page && ['dashboard','profile','ranking','transfer','chat','hall','metaverse'].includes(page)) {
        await initProtectedPage();
      }
    });
  }

  // 3️⃣ تطبيق الترجمات على كل النصوص
  applyTranslations(document);
