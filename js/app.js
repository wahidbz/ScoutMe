import { loadLanguage, setupLanguageSwitcher, t, applyTranslations, getCurrentLanguage } from '/js/i18n.js';
import { ensureFirebaseSession, loginWithPi, logout, requireProfile, updatePresence } from '/js/auth.js';
...
import { initRankingPage } from '/js/ranking.js';
import { initTransferPage } from '/js/transfer.js';
import { initChatPage } from '/js/chat.js';
import { initMetaversePage } from '/js/metaverse.js';
...
const renderLogin = async () => {
  ...
  button?.addEventListener('click', async () => {
    try {
      ...
      location.href = '/public/profile.html?onboarding=1';
    } catch (error) {
  ...
  <div class="dashboard-grid">
    ...
    <aside class="glass-card span-5">
      ...
      <div class="feed-list">
        ...
        <a class="ghost-btn" href="/public/transfer.html">${t('dashboard.goTransfers', 'Open transfer board')}</a>
      </div>
    </aside>
  ...
  const renderPools = () => {
    ...
    q('#dashboard-trending').innerHTML = trending.map((user) => `
      <article class="player-card">
        ...
        <a class="ghost-btn" href="/public/profile.html?uid=${user.uid}">${t('common.viewProfile', 'View profile')}</a>
      ...
      <div class="profile-grid">
        ...
        <aside class="glass-card">
          ...
          <div class="feed-list">
            ...
            <a class="ghost-btn" href="/public/chat.html?uid=${viewingUid}">${t('profile.openChat', 'Open chat')}</a>
          </div>
        </aside>
      </div>
    `;
  ...
  const render = () => {
    ...
    q('#hall-list').innerHTML = items.map((entry) => `
      <article class="hall-card">
        ...
        <a class="ghost-btn" href="/public/profile.html?uid=${entry.playerId}">${t('common.viewProfile', 'View profile')}</a>
