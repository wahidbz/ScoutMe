import { listenAllUsers, listenTalents, syncUserScore } from '/js/database.js';
import { buildScoreRing, calculateScore, COUNTRY_OPTIONS, formatDate, numberFormat, q, SPORTS } from '/js/utils.js';
...
export const initRankingPage = async ({ currentProfile }) => {
  ...
  const render = async () => {
    ...
    q('#ranking-body').innerHTML = combined.map((user, index) => `
      <tr>
        <td>${index + 1}</td>
        <td><a href="/public/profile.html?uid=${user.uid}">${user.username}</a></td>
        <td>${user.sport}</td>
